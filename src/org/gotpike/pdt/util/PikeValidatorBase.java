 ...
 * @author jploski
 */
abstract class PikeValidatorBase
{
	...
	private final PikeValidatorErrors errors;
    private final PikeExecutor executor;
	
    protected PikeValidatorBase(ILog log, PikeExecutor executor)
    {
        this.log = log;
        this.executor = executor;
        
        errors = new PikeValidatorErrors();
    }
    ...
    {
        String perlOutput = runPike(resource, sourceCode);

        if (DEBUG) printPikeOutput(perlOutput);
        ...
        {                
            ...
            PikeValidatorErrors.ErrorMessage errorMsg =
			...
			attributes.put(
                Constants.MARKER_ATTR_PIKE_ERROR_EXPLANATION,
     ...
     */
    private static List makeLinesList(String pikeOutput)
    {
        List lines = new ArrayList();
        StringTokenizer st = new StringTokenizer(pikeOutput, "\r\n");
        int lineCount = 0;
    ...
    private void printPikeOutput(String pikeOutput)
    {
        if (pikeOutput.indexOf("syntax OK") == -1)
        {
			System.out.println("-----------------------------------------");
    ...
    /**
     * Executes the Pike interpreter to validate the given script.
     * 
     * @param resource    source file
     * @param sourceCode  text of the source file 
     * @return stderr output of the Perl interpreter
     */
    private String runPike(IResource resource, String sourceCode)
        throws CoreException
    {
        return executor.execute(resource, getPikeArgs(), sourceCode).stderr;
    }       
        ...
        catch (BadLocationException e)
        {
            ...
            log.log(
                new Status(Status.ERROR,
                    PDTPlugin.getPluginId(),
                    IStatus.OK,
                    "Unexpected exception in PikeValidator.underlineError: " +
            ...
            catch (NumberFormatException e)
            {
                ...
                log.log(
                    new Status(Status.ERROR,
                        PDTPlugin.getPluginId(),
                        IStatus.OK,
                        "Could not parse line number contained in Pike " +
