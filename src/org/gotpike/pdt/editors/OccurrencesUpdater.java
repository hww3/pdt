...
public class OccurrencesUpdater implements ISelectionChangedListener
{
    ...
    private static final String ANNOTATION_TYPE = "org.gotpike.pdt.occurrence";
    ...
    private boolean shouldMarkContentType(String contentType)
    {
        ...
        if (contentType.equals(PartitionTypes.AUTODOC))
        {
            if (!getBoolPref(MarkOccurrencesPreferences.AUTODOC))
            {
                return false;
            }
        }
    ...
    {
        return PDTPlugin.getDefault().getPreferenceStore().getBoolean(name);
    }
